...
def fig_to_base64(fig) -> str:
    buf = io.BytesIO()
    fig.savefig(buf, format="png", bbox_inches="tight", dpi=120, facecolor="#1E1A22")
    buf.seek(0)
...
def generate_charts(df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        # Plot top representative regions to keep chart clean
        top_regions = df["location_id"].unique()[:10]
        sub_df = df[df["location_id"].isin(top_regions)]
        ...
        ax.set_title("주요 거점별 기온(℃) 24시간 실시간 변화 추이", fontsize=13, pad=12, color="#E2E8F0", fontweight="bold")
    ...
    try:
        ...
        ax2.set_title("주요 지역별 평균 습도 (%)", fontsize=11, color="#E2E8F0")
        ax2.set_xlabel("")
    ...
    # 3. Correlation Heatmap with Korean Axis Labels
    try:
        col_mapping = {
            "temperature": "기온 (℃)",
        ...
        if len(avail_cols) > 1:
            ...
            sns.heatmap(corr, annot=True, fmt=".2f", cmap="vlag", ax=ax, cbar=True, linewidths=0.8, linecolor="#2D2534",
                        annot_kws={"size": 10, "weight": "bold"})
            ax.set_title("기상 관측 변수 간 상관계수 히트맵", fontsize=13, pad=12, color="#E2E8F0", fontweight="bold")
...
<body class="p-6 md:p-10 max-w-7xl mx-auto min-h-screen">
    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center pb-8 border-b border-gray-800 gap-4">
        <div>
            ...
            <p class="text-gray-400 text-sm mt-1.5">DVC 무중단 파이프라인 자동 산출물 | 대상 일자: <strong class="text-amber-400">{target_date}</strong> (KST 00:00:00 자동 생성)</p>
        </div>
        <div class="flex items-center gap-3">
    ...
    <section class="grid grid-cols-2 md:grid-cols-4 gap-5 my-8">
        <div class="glass-card rounded-2xl p-6">
            ...
            <div class="text-xs text-gray-400 mt-1.5">{quality_metrics.get('total_regions', 0)}개 관측소 1분 수집</div>
        </div>
        <div class="glass-card rounded-2xl p-6">
    ...
    <section class="space-y-8 my-8">
        <div class="glass-card rounded-2xl p-6">
            <h3 class="text-lg font-bold text-gray-200 mb-4">📈 전국 주요 거점 24시간 기온 시계열 추이</h3>
            {"<img src='" + charts["temp_timeseries"] + "' class='w-full rounded-xl border border-gray-800' />" if "temp_timeseries" in charts else "<div class='text-gray-500 py-8 text-center'>차트 데이터 없음</div>"}
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="glass-card rounded-2xl p-6">
                <h3 class="text-lg font-bold text-gray-200 mb-4">📊 주요 지역별 기온 & 습도 분포 비교</h3>
                {"<img src='" + charts["regional_comparison"] + "' class='w-full rounded-xl border border-gray-800' />" if "regional_comparison" in charts else "<div class='text-gray-500 py-8 text-center'>차트 데이터 없음</div>"}
            </div>
            <div class="glass-card rounded-2xl p-6">
                <h3 class="text-lg font-bold text-gray-200 mb-4">🔥 기상 관측 변수 간 상관분석 (히트맵)</h3>
                {"<img src='" + charts["correlation_heatmap"] + "' class='w-full rounded-xl border border-gray-800' />" if "correlation_heatmap" in charts else "<div class='text-gray-500 py-8 text-center'>차트 데이터 없음</div>"}
            </div>
        </div>
    </section>
